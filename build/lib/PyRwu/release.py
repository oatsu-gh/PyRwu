...
RELEASE_FILE = os.path.join(
    "..",
    "release",
    PROJECTNAME + "-" + datetime.datetime.strftime(datetime.datetime.now(), "%Y%m%d"),
)
...
EXE_DIRS = [
    os.path.join("..", "PyRwuExe", "bin", "Release", "net6.0-windows"),
    os.path.join("..", "PyRwuDirty", "bin", "Release", "net6.0-windows"),
]
EXE_DIR_EXTENSIONS = [".exe", ".dll", ".runtimeconfig.json"]
SOURCE_FILES = []

for pathname, _dirnames, filenames in os.walk("."):
    if "env" in pathname:
        continue
...
for file in SOURCE_FILES:
    ...
    os.makedirs(
        os.path.join(PYPI_DIR, "src", "PyRwu", os.path.dirname(file)), exist_ok=True
    )
...
if not os.path.isdir(os.path.join(BUILD_DIR, PYTHON)):
    ...
    shutil.unpack_archive(
        os.path.join(BUILD_DIR, PYTHON + ".zip"), os.path.join(BUILD_DIR, PYTHON)
    )
