...
def read(input_path: str, offset: float, end_ms: float) -> tuple[np.ndarray, int]:
    ...
    try:
        with wave.open(input_path, "rb") as wr:
            ...
            bytes_data: bytes = wr.readframes(nframes)
    except Exception as e:
        raise TypeError(f"{input_path} can't read. this file isn't wave format.") from e
    ...
    bytes_data = bytes_data[
        offset_frame * sampwidth * channels : end_frame * sampwidth * channels
    ]
    ...
    if sampwidth == 1:
        ...
        for i in range(int(len(bytes_data) / sampwidth)):
            data[i] = int.from_bytes(
                bytes_data[i * sampwidth : (i + 1) * sampwidth], "little", signed=True
            )
    elif sampwidth == 4:
...
def write(
    output_path: str, data: np.ndarray, framerate: int = 44100, sampwidth: int = 2
):
    """
    | waveファイルを保存します。
