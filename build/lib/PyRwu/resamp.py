...
class Resamp:
    ...
    def _getAp(
        ...
        if not settings.USE_D4C_FILE:
            ...
            size: int = (
                int(pw.get_cheaptrick_fft_size(self._framerate, f0_floor) / 2) + 1  # pyright: ignore[reportAttributeAccessIssue]
            )
            ...
            f0 = pw.stonemask(input_data, f0, t, framerate)  # pyright: ignore[reportAttributeAccessIssue]
            self._ap = pw.d4c(input_data, f0, t, framerate, threshold=threshold)  # pyright: ignore[reportAttributeAccessIssue]
            self._ap.tofile(d4c_path)
            start_frame: int = int(self._offset / frame_period)
            self._ap = self._ap[start_frame:][: self._f0.shape[0]]

    def _getInputFromNpz(
        self,
        f0_floor: float = settings.PYWORLD_F0_FLOOR,
        f0_ceil: float = settings.PYWORLD_F0_CEIL,
        frame_period: float = settings.PYWORLD_PERIOD,
    ...
    def stretch(self) -> None:
        ...
        if self._velocity != 100:
            ...
            vel_f0, vel_sp, vel_ap = stretch.world_stretch(
                self._fixed_frames,
        ...
        if self._target_frames - self._fixed_frames > 0:
            if self.flags.params['e'].flag:
                ...
                s_f0, s_sp, s_ap = stretch.world_loop(
                    self._target_frames - self._fixed_frames,
                    self._f0[input_fix_frames:],
                    self._sp[input_fix_frames:],
                    self._ap[input_fix_frames:],
        ...
        self._t = np.arange(
            0, self._f0.shape[0] * settings.PYWORLD_PERIOD, settings.PYWORLD_PERIOD
        )[: self._target_frames]

    def pitchShift(self) -> None:
        """
        ...
        average_frq: float = float(np.average(self._f0))
        ...
        if self._modulation != 100:
            mod_f0: np.ndarray = np.stack(
                [self._f0, np.full(self._f0.shape[0], self._target_frq)]
            )
            self._f0 = np.average(
                mod_f0,
                axis=0,
                weights=[self._modulation / 100, 1 - self._modulation / 100],
            )

    def applyPitch(self) -> None:
        """
        ...
        utau_t: np.ndarray = pitch.getPitchRange(
            self._tempo, self._target_ms, self._framerate
        )
