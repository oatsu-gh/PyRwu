...
def read(
    input_path: str, offset: float, end_ms: float, framerate: int, world_period: float
    ...
    base_data: np.ndarray = np.frombuffer(bytes_data, dtype="float64")[
        ::2
    ]  # 音量はいらない
    ...
    frq_t: np.ndarray = np.arange(0, frq_span * (base_data.shape[0] + 1), frq_span)[
        : base_data.shape[0]
    ]
...
def write(wav_data: np.ndarray, output_path: str, framerate: int):
    f0, t = pw.harvest(wav_data, framerate, frame_period=1000 / 44100 * 256)  # pyright: ignore[reportAttributeAccessIssue]
    f0 = pw.stonemask(wav_data, f0, t, framerate)  # pyright: ignore[reportAttributeAccessIssue]
