...
class LargeAFlag(WorldEffectBase):
    @staticmethod
    def apply(params) -> np.ndarray:
        ...
        frame_step: int = int(
            round(60 / 96 / bpm * params.framerate, 0)
        )  # ピッチ列1つあたりが影響を与えるwavフレーム数
        pitch_length: int = int(nframes / frame_step) + 1
        wave_t: np.ndarray = np.arange(0, frame_step * (pitch_length + 1), frame_step)[
            : pitch_length + 1
        ]
        if wave_t.shape[0] > params.pitches.shape[0]:
            pitches: np.ndarray = np.pad(
                params.pitches, (0, wave_t.shape[0] - params.pitches.shape[0])
            )
        else:
        ...
        effect: np.ndarray = (
            interpolate.interp1d(
                wave_t, np.arange(params.output_data.shape[0]), pitches
            )
            / 100
        )
